...
Conditioned on Species and Sex",
        xlab = "Species : Sex",
        ylab = "Body Mass",
        names = c("D. dorsalis:female", "D. sublineatus:female", "D. dorsalis:male", "D. sublineatus:male"))
...
However, in the graphical exploration, we can see that there are normal patterns for the last four models. For the first model, although there is a peak of amount of residuals and the amount tends to decrease in both tails, the values are highly concentrated in two ranges and we can find extreme values located to the left of the distribution.

There is a conflict between the numerical and the graphical diagnostics. However, since most populations tend to get normally distributed as its size increases, I would say it would be safe to assume a normal distribution for the residuals of the last four models. I would not assume a residual normality for the first model, since both the numerical and graphical explorations do not show normality.
